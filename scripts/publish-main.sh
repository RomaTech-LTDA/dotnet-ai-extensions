...
PACKAGE_NAME="Romatech.Extensions.Ai"
...
PKG=$(find "$OUTPUT_DIR" -name "${PACKAGE_NAME}.1*.nupkg" -o -name "${PACKAGE_NAME}.0*.nupkg" | grep -v "Mcp\|Rag\|Metadata\|Swagger\|Shared" | sort -V | tail -1)

if [ -z "$PKG" ]; then
  echo "⚠️  Package not found. Running pack first..."
  "$SOLUTION_ROOT/scripts/pack-main.sh"
  PKG=$(find "$OUTPUT_DIR" -name "${PACKAGE_NAME}.1*.nupkg" -o -name "${PACKAGE_NAME}.0*.nupkg" | grep -v "Mcp\|Rag\|Metadata\|Swagger\|Shared" | sort -V | tail -1)
fi
